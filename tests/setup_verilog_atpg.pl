...
sub create_atpg_makefile {
    ...
    print $fh "FAN_TARGETS = ", join(" ", map { "$_.fan" } @stems), "\n";
    ...
    print $fh "\t\@\$(FAN_ATPG) -i \$< -o \$*.fan.rpt 2>&1 | tee \$*.fan.log || true\n";
    print $fh "\t\@touch \$@\n";
    print $fh "\n";
    ...
    for (my $i = 0; $i < @stems; $i++) {
        ...
        print $fh "\n";
        print $fh "$stem.fan: $file\n";
        print $fh "\t\@echo \"FAN_ATPG: Testing $file...\"\n";
        print $fh "\t\@\$(FAN_ATPG) -i $file -o $stem.fan.rpt 2>&1 | tee $stem.fan.log || true\n";
        print $fh "\t\@touch \$@\n";
    }
